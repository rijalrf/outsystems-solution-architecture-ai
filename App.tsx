...
const App: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    if (!file) {
      setError(t("error.selectFile"));
      return;
    }
    if (!apiKey) {
      setError(t("apiKeyModal.error"));
      setIsApiModalOpen(true);
      return;
    }
  ...
  const handleSaveApiKey = (key: string) => {
    try {
      ...
      setError(
        "Could not save API key. Your browser might be in private mode or has storage disabled."
      );
    }
  };
